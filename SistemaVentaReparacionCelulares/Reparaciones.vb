...
Public Class Reparaciones
    ...
    Private _reparacionID As String = ""
    Private _reparacionFecha As String = ""
    ...
    Public Sub insertarFacturaReparacion()
        Dim conector As New Conexion()
        Dim facturaID As Integer = conector.InsertarFacturaReparacion(Me._TotalSubTotal, Me._TotalITBISTotal, Me._TotalMontoTotal, Me._ClienteID, Me._usuarioID)
        conector.Cerrar()
        If facturaID > 0 Then
            Me.insertarFacturaReparacionDetalle(facturaID.ToString())
            If Me._MontoPago > 0 Then
                Me.insertarPago(facturaID.ToString())
            End If
            Me.obtenerFacturaReparacion(facturaID.ToString())
        End If
    End Sub
    Public Sub insertarFacturaReparacionDetalle(facturaID As String)
        Dim conector As New Conexion()
        For i As Integer = 0 To Me.ArrayListProductos.Count - 1 Step 1
            Dim productos As ModeloReparacionReporte = Me.ArrayListProductos(i)
            conector.InsertarFacturaReparacionDetalle(facturaID, productos.Codigo, productos.Cantidad, productos.Total, productos.Precio, Me._usuarioID, productos.Problema)
        Next
        conector.Cerrar()
    End Sub
    Public Sub insertarPago(facturaID As String)
        Dim conector As New Conexion()
        conector.InsertarPago("0", facturaID, "reparacion", Me._MontoPago.ToString(), Me._DevueltaPago.ToString(), Me._usuarioID, Me._TipoPago)
        conector.Cerrar()
    End Sub
    Public Sub obtenerFacturaReparacion(facturaID As String)
        Dim conector As New Conexion()
        Dim array() As String = conector.ObtenerDataFacturaReparacion(facturaID)
        Me._reparacionID = array(0)
        Me._reparacionFecha = array(1).Split(" ")(0)
        conector.Cerrar()
    End Sub
    Public Sub VerReporte(guardar As Boolean)
        ...
        If Me.validarClienteProducto Then
            If guardar Then
                Me.insertarFacturaReparacion()
                Me.ReFormatearDecimales()
            End If
            Dim reporte As New ReportDocument()

            reporte.Load("C:\Diplomado\SistemaVentaReparacionCelulares\SistemaVentaReparacionCelulares\FacturaReparacion.rpt")
            'reporte.SetDataSource(Me.ArrayListProductos)
            'reporte.SetParameterValue("SubTotal", Me._Sub_Total.ToString())
            ...
            reporte.SetDataSource(Me.ArrayListProductos)
            reporte.SetParameterValue("Fecha", Me._reparacionFecha.ToString())
            reporte.SetParameterValue("NoFactura", Me._reparacionID.ToString())
    ...
    Public Sub ReFormatearDecimales()
        For i As Integer = 0 To Me.ArrayListProductos.Count - 1 Step 1
            Dim Producto As ModeloReparacionReporte = Me.ArrayListProductos(i)
    ...
    Public Sub Agregar()
        Try
            ...
            ElseIf Me.TextBox9.Text = "" Then
                MessageBox.Show("Esta vacío el problema del producto")
            ...
            Else
                ...
                Me._Sub_Total += total
                Me.ArrayListProductosCalcular.Add(New ModeloReparacionReporte(Me._CodigoProducto, Me.TextBox1.Text, Me.TextBox3.Text, Me.TextBox2.Text, total, Me.TextBox9.Text))
                Me.ArrayListProductos.Add(New ModeloReparacionReporte(Me._CodigoProducto, Me.TextBox1.Text, Me.TextBox3.Text, Me.TextBox2.Text, total, Me.TextBox9.Text))
                Me.limpiar()
    ...
    Private Sub TextBox1_Click(sender As Object, e As EventArgs) Handles TextBox1.Click
        ...
        buscador.setReparaciones(Me)
        buscador.setOrigen("reparaciones")
        buscador.Show()
    End Sub
    ...
    Public Sub cargarGridViewProductosVender()
        ...
        For i As Integer = 0 To Me.ArrayListProductosCalcular.Count - 1 Step 1
            Dim Producto As ModeloReparacionReporte = Me.ArrayListProductosCalcular(i)
            'MessageBox.Show("c " + Producto.Codigo + " N " + Producto.Producto)
            subTotal += Double.Parse(Producto.Total)
            Me.DataGridView1.Rows.Add(Producto.Codigo, Producto.Producto, Producto.Precio, Producto.Cantidad, Producto.Total, Producto.Problema)
        Next
    ...
    Private Sub Button4_Click(sender As Object, e As EventArgs) Handles Button4.Click
        If Me.validarClienteProducto Then
            ...
            pagos.setReparaciones(Me)
            pagos.setOrigen("reparaciones")
            pagos.Show()
        End If
    End Sub
